...
- **ASR provider (product decision 2026-09-06, option A):** primary local ASR is Google's official **ML Kit GenAI Speech Recognition API in Advanced mode** (Gemini Nano via the AICore Android system service; natively supported on Pixel 10 and Pixel 11). Capture is now **schedule-gated** (default weekdays 08:00–17:00, user-editable, with a manual override toggle) but the capture *stack* is unchanged (passive mic → VAD → 16k mono segments → on-device raw audio, 30-day Opus48k rolling window); transcription is **foreground-gated** — while the app is open, capture feeds the queue live and the backlog drains live-while-open; when the app is not foreground, transcription pauses and scheduled capture keeps running. The previous on-device candidates (Parakeet/LiteRT int8, sherpa-onnx CPU) are **demoted to parked plan-B**: their code and benchmark harness remain in the tree, marked parked, not deleted.
