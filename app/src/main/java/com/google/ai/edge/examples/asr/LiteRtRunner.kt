...
class LiteRtRunner(
  ...
  /**
   * AOT Google Tensor packs must run NPU-only. Including CPU as a companion accelerator
   * (stock sample default) often breaks invoke on Pixel 10 / Tensor G5 AOT models.
   */
  ...
  private val compiledModel: CompiledModel =
    {
      // Stock sample always includes CPU as a fallback. For AOT Tensor G5, prefer NPU-only.
      val accelerators =
        if (accelerator == Accelerator.NPU && npuOnly) {
          setOf(Accelerator.NPU)
        } else {
          setOf(Accelerator.CPU, accelerator)
        }
      ...
      if (accelerator == Accelerator.GPU) {
        ...
        // Qualcomm HTP options only apply on Snapdragon; skip for Google Tensor.
