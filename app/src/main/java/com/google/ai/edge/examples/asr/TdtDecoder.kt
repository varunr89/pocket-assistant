...
class TdtDecoder(private val compiledModel: CompiledModel, private val modelConfig: ModelConfig) :
  ...
  override fun decode(encodeOutputBuffers: List<TensorBuffer>): Sequence<Pair<Int, Int>> =
    sequence {
      // Each recognize() window must start from a clean RNN state index.
      // Upstream sample leaves this sticky across windows; that breaks multi-chunk file ASR.
      inputStatesBuffersIndex = 0
